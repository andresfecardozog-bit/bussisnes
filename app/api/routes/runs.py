...
def start_batch(
    ...
    master_id = create_run(conn, status="running", current_step="batch_created")
...
def get_run_endpoint(run: dict = Depends(get_run_or_404)) -> RunSummary:
...
def list_runs(
    ...
    return [_row_to_summary(r) for r in list_recent_runs(conn, limit=limit)]
